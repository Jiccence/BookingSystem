package com.tsc.booking.filter;
...
import com.tsc.booking.utils.CookieUtils;
public class AutoLoginFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		// 2.1判断当前用户是否登录
		TeaBean tuser = (TeaBean) req.getSession().getAttribute("tuser");
		StuBean suser = (StuBean) req.getSession().getAttribute("tuser");
		
		if (tuser == null||suser==null) { 
			// user为null说明用户没有登录，可以进行自动登录操作
			// 2.2 得到访问的资源路径
			...
			if (!("/regist.jsp".equalsIgnoreCase(path)
				...
				if (cookie != null) {
					...
					try {
						StuBean existUser = service.Slogin(username, password);
						if (existUser != null) {
							// 可以进行登录操作
